...
class Algo(object):
    ...
    def _convert_prices(self, S, method, replace_missing=False):
        ...
        if method == 'raw':
            ...
            return S.pct_change() + 1  # pumpkin 每日收益率

        elif method in ('ratio', 'log', 'ratio_1'):
            # be careful about NaN values

            # S.shift（1）将S系列或DataFrame的值向前移动一个位置
            # S / S.shift(1) performs element-wise division between S and the shifted version of S.
            X = S / S.shift(1).fillna(method='ffill')  # fillna（）方法用于处理生成的Series或DataFrame中缺少的值。
